...
export default {
  ...
  computed: {
    ...
    timeFormatter() {
      if (!this.destinationDetails?.openTime || !this.destinationDetails?.closeTime) {
        return "Unavailable";
      }
      else {
        return `${this.destinationDetails?.openTime.slice(0, 5)} - ${this.destinationDetails?.closeTime.slice(0, 5)}`;
      }
    },
  ...
  methods: {
    ...mapActions(useMainStore, ['fetchDestinationDetails', 'handleSubmitReview', 'handleAddToSchedule', 'fetchGooglePlaceImages']),
...
<template>
  ...
  <main class="w-4/5 mx-auto mt-8 min-h-screen ">
    ...
    <h1 class="text-center text-4xl font-bold mb-4">{{ destinationDetails?.name }}</h1>
    <div class="grid grid-cols-2 gap-16 mb-4 ">
      <div>
        <h2 class="text-left text-2xl font-bold mb-4 underline underline-offset-8">About</h2>
    ...
    <div>
      <h2 class="text-2xl font-bold mb-4 underline underline-offset-8">Informations</h2>
      <div class="grid grid-cols-6 gap-2 mb-2 items-center">
        ...
        <div class="flex">
          ...
          <p class="ml-2">{{ destinationDetails?.city }}, {{ destinationDetails?.country }}</p>
        </div>
        <div class="flex">
    ...
    <div class=" w-full origin-top ease-in-out forwards duration-[600ms]"
      ...
      <form @submit.prevent="addToSchedule">
        ...
        <p class="mb-2">Do you want to add it to Google Calendar? <span class="text-sm">(*You might be asked to sign in
            first)</span></p>
        <div class="mb-2">
    ...
    <div class="bg-slate-200 w-full h-[40rem] mb-8 ease-in-out forwards duration-300">
      ...
      <div class="px-8 py-4 h-1/4 overflow-x-auto flex justify-left">
        <div v-for="(image, index) in imageUrls" @click.prevent="changeImage(index)"
    ...
    <div class="">
      <h2 class="text-2xl font-bold mb-4 underline underline-offset-8">Find On Google Maps</h2>
      <p class="text-left mb-2"><span class="font-bold">Address: </span>{{ destinationDetails?.address }}</p>
    </div>
    <GoogleMap v-if="showGoogleMaps" :lat="destinationDetails?.latitude" :lng="destinationDetails?.longitude" />
    <!-- Reviews Container -->
    <div class=" flex">
      <h2 @click.prevent="changeReviewTab('view')" :class="[showAddReview ? '' : ' underline-offset-8 underline']"
        class="text-2xl font-bold mb-4 pr-8 hover:cursor-pointer">Reviews</h2>
      <h2 @click.prevent="changeReviewTab('add')" :class="[showAddReview ? ' underline-offset-8 underline' : '']"
        class="text-2xl font-bold mb-4 pr-8 hover:cursor-pointer">Add Your Review</h2>
    </div>
    <!-- Reviews -->
    <div class="" v-if="showAddReview">
      <form @submit.prevent="submitReview">
        ...
        <select v-model="rating" class="border-2 border-slate-600 border-solid px-2 py-1 mb-4" name="" id="">
        ...
        <p></p>
        <button class=" text-white font-semibold px-4 py-2 bg-green-400 hover:bg-green-500 rounded-full mb-4"
          type="submit">
          Submit Your Review
        </button>
      </form>
