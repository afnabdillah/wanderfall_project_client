...
export default {
  ...
  computed: {
    ...mapState(useMainStore, ["destinations", "loginStatus"]),
  },

  methods: {
    ...mapActions(useMainStore, ["fetchDestinations", "handleAuthClick", "handleSignoutClick" ]),
    ...
    goToDestination(destination) {
      this.$router.push(
        `/destinations/${destination.name.toLowerCase().split(" ").join("-")}-${
          destination.id
        }`
      );
    },
  ...
  <main class="min-h-screen">
    ...
    >
      <div class="w-1/2 mx-auto text-center">
        <h1
          class="text-[3.5rem] font-semibold my-4 drop-shadow-[0_1.2px_1.2px_rgba(0,0,0,0.8)]"
        >
          Wander Fall
        </h1>
        <p class="text-xl drop-shadow-[0_1.2px_1.2px_rgba(0,0,0,0.8)] mb-2">
          Are you having trouble to create your holiday plan? Say no more!
          Wander Fall will help you create the perfect plan for your holiday to
          make sure you get the most experience!
        </p>
        ...
        <p class="mb-4 text-xl drop-shadow-[0_1.2px_1.2px_rgba(0,0,0,0.8)]">
          Or maybe you want to start right away!
        </p>
        <div v-if="!loginStatus" class="w-1/2 mx-auto flex justify-around">
          <router-link
            to="/login"
            class="text-xl bg-green-400 hover:bg-green-500 hover:shadow-xl px-6 py-1 rounded-full drop-shadow-[0_1.2px_1.2px_rgba(0,0,0,0.8)]"
            >Log In</router-link
          >
          <router-link
            to="/signup"
    ...
    <div class="">
      ...
      <div class="w-4/5 mx-auto mb-10">
        <div class="relative slide">
          <!-- Carousel indicator -->
          <div
            class="carousel-indicators absolute bottom-0 flex h-8 w-full justify-center items-center"
          >
            <ol class="z-50 flex w-4/12 justify-center">
              <li
                v-for="(destination, i) in destinations.destinations"
                :key="i"
          ...
          <div class="carousel-inner relative overflow-hidden w-full h-[36rem]">
            ...
            >
              ...
              <img
                @click.prevent="goToDestination(destination)"
                class="block object-cover w-full h-full hover:cursor-pointer"
                :src="destination.Images[0].imgUrl"
                alt="First slide"
              />
            </div>
          </div>
